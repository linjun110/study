define(['text!templates/foodsCompositeViewTemplate.html', 'views/foodView', 'models/foodModel'],function(Template, ItemView, ItemModel){
	...
	view = Backbone.Marionette.CompositeView.extend({
		...
		appendHtml: function(collectionView, itemView){ //内置方法, called for each item
			var _this = this;
			itemView.on("foodEvent", _this.bookEventHandler, _this);
		...
		bookEventHandler: function (e) {
			 ...
			 */
			 if(e.event === "clicked"){
			 	...
			 	this.$el.find("#name").val(_model.get("name"));
			 	this.$el.find("#desc").val(_model.get("desc"));
		...
		addHandler: function () {
			...
			_model.set("name", _this.$el.find("#name").val());
			_model.set("desc", _this.$el.find("#desc").val());
			_model.save({},{
